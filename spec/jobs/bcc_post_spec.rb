  ...
  context "when enabled" do
    ...
    it "works when only using emails" do
      ...
      ::Jobs::BccPost.new.execute(
        ...
        create_params: create_params,
        targets_key: "target_emails",
        targets: %w[test@test.com test2@test.com],
      )
    ...
    it "works with standard personalization" do
      ::Jobs::BccPost.new.execute(
        user_id: sender.id,
        create_params:
          create_params.merge(raw: "this is the content I want to send to %{username}"),
        targets_key: "target_usernames",
    ...
    it "works with name personalization" do
      ::Jobs::BccPost.new.execute(
        user_id: sender.id,
        create_params: create_params.merge(raw: "this is the content I want to send to %{name}"),
        targets_key: "target_usernames",
        targets: [user1.username],
      )
